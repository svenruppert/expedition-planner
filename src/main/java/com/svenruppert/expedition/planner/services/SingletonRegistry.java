...
public class SingletonRegistry implements HasLogger {
  ...
  public static ParticipantsService getOrCreateParticipantsService() {
    if (INSTANCES.containsKey(ParticipantsService.class)) {
      return getInstance(ParticipantsService.class);
    } else {
      ParticipantRepository participantRepository = getOrCreateParticipantRepository();
      ParticipantsService participantsService = new ParticipantsService(participantRepository);
      registerInstance(ParticipantsService.class, participantsService);
